        ...
        try {
            ...
            try {
                ...
                if (queryPacket == null) {
                    ...
                    this.packetSequence = -1;				// TODO 啥意思?
                    this.compressedPacketSequence = -1;		// TODO 啥意思?
    ...
    throws Exception {
    	...
    	try {
	    	...
	    	if (query != null) {
	    		...
	    		// 这里的计算length感觉和原来英文注释已经不符了。没明白为什么有这样一个公式 TODO.
	    		int packLength = HEADER_LENGTH + 1 + (query.length() * 3) + 2;
 
	    		// 处理注释部分
	    		byte[] commentAsBytes = null; 
	    		if (statementComment != null) {
	    			commentAsBytes = StringUtils.getBytes(
	    					statementComment, 
	    					null,
	    					characterEncoding, 
	    					this.connection.getServerCharacterEncoding(),
	    		...
	    		if (this.sendPacket == null) {
	    			...
	    			// 这个clear有点迷惑，他实际上并不是清空，而是把position移动到4.4刚好是header的位置末尾。
	    			// 也就是说意思上是除了header全部clear。
	    			this.sendPacket.clear();
	    		}
	    		...
	    		if (characterEncoding != null) {	// 默认的编码characterEncoding=Cp1252，从mysql服务器取来
	    			if (this.platformDbCharsetMatches) {		// 这个值不知道怎么拿来的TODO
	    				// 写入sql的bytes[]到packet
	    				this.sendPacket.writeStringNoNull(
	    						query, 
	    						characterEncoding,
	    						this.connection.getServerCharacterEncoding(),		// latin1
	    						this.connection.parserKnowsUnicode(),				// true
	    						this.connection);
	    			} else {
	    	...
	    	// Send query command and sql query string 这里应该是最核心的地方了 发送查询命令
	    	...
	    	ResultSetInternalMethods rs = readAllResults(
								    			callingStatement, 
								    			maxRows, 
								    			resultSetType,
								    			resultSetConcurrency,
